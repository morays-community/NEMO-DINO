...
!         ENDIF
      ...
      IF( ssnd(ntypsbc,jps_soce  )%laction )  THEN
         CALL cpl_snd( jps_soce  , isec, ntypsbc, CASTSP(RESHAPE ( ts(:,:,1,jp_sal,Kmm), (/jpi,jpj,1/) )), info )
      ENDIF
      !                                                        ! first T level thickness
      IF( ssnd(ntypsbc,jps_e3t1st )%laction )  THEN
 CALL cpl_snd( jps_e3t1st, isec, ntypsbc, CASTSP(RESHAPE ( e3t(:,:,1,Kmm) , (/jpi,jpj,1/) )), info )
      ENDIF
      !                                                        ! Qsr fraction
